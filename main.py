...
async def ban_handler(event):
    ...
    if event.reply_to_msg_id:
        ...
        if user_input.isdigit():
            ...
            await event.respond('❌ Galat format bhai!\n\n📌 Sahi format: `/ban <user_id>` ya `/ban @username` ya msg pe reply karke `/ban`')
            raise events.StopPropagation
    else:
        await event.respond('❌ Koi user specify nahi kiya bhai!\n\n📌 Use: `/ban <user_id>` ya `/ban @username` ya msg pe reply karke `/ban`')
        raise events.StopPropagation
    ...
    if target_user.get('banned'):
        ...
        try:
            await client.send_message(target_user['user_id'], f'🚫 Tujhe BAN kar diya gaya hai{group_name}! ❌')
        except Exception:
            pass
...
async def num_handler(event):
    ...
    # Auto-delete response after 3 minutes
    await schedule_message_delete(processing_msg, 180)
