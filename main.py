...
async def check_tool_group_access(event):
    ...
    if not event.is_group:
        return False, "🚫 Tools केवल authorized groups में काम करते हैं!\n\nपहले एक group join करें जहां यह bot added है।"
...
def extract_json_fields(data, fields):
    ...
    if has_paths:
        # Extract using dot notation paths
        ...
        if isinstance(data, dict):
            # Check if there's a 'data' key with array
...
async def callback_handler(event):
    ...
    if sender.id != owner_id:
        ...
        allowed_for_users = data.startswith(b'user_') or data == b'check_subscription'
...
async def message_handler(event):
    ...
    if sender.id in tool_api_action:
        ...
        field_example = '''📝 **STEP 2: Response Field Mapping**

API URL saved! Now specify which fields to show from JSON response.

**Example API Response:**
```json
{
  "success": true,
  "data": [{"name": "John", "mobile": "123"}],
  "developer": "ABC"
}
```

**Field Formats:**
• Simple: `name, mobile, address`
• Nested: `data.0.name, data.0.mobile`

**Send field names** (comma separated):
Example: `name, mobile, father_name, address`

Or type **"skip"** to show full JSON response.'''
    ...
    if channel_action_temp.get(sender.id) == 'add':
        ...
        if channel_exists(ch_name):
            ...
            add_channel(ch_name, ch_title, ch_id)
...
async def num_handler(event):
    ...
    allowed, msg = await check_tool_group_access(event)
    if not allowed:
