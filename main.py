...
async def delwarn_handler(event):
    ...
    try:
        ...
        await event.respond(f'❌ Message delete nahi ho paya: {str(e)[:50]}')
        raise events.StopPropagation
    ...
    if new_count >= 3:
        try:
            await client.edit_permissions(chat, target_user_id, view_messages=False)
            clear_warnings(chat.id, target_user_id)
            warn_text = f"🗑️ Message delete + 🚨 **AUTO BAN!**\n\n👤 {target_name} ({target_username})\n⚠️ 3 warnings complete!\n🔨 Group se ban!"
            if reason:
                warn_text += f"\n📝 Reason: {reason}"
        except:
            warn_text = f"🗑️ Message delete + ⚠️ **WARNING {new_count}/3**\n\n👤 {target_name}"
    else:
        warn_text = f"🗑️ Message delete + ⚠️ **WARNING {new_count}/3**\n\n👤 {target_name} ({target_username})"
        if reason:
            warn_text += f"\n📝 Reason: {reason}"
...
async def unwarn_handler(event):
    ...
    await event.respond(f"✅ Warning hata di!\n\n👤 {target_name} ({target_username})\n⚠️ Current warnings: {new_count}/3")
    raise events.StopPropagation
...
async def mute_handler(event):
    ...
    try:
        from telethon.tl.types import ChatBannedRights
        await client.edit_permissions(chat, target_user_id, ChatBannedRights(
            until_date=None,
            send_messages=True
        ))
...
async def unmute_handler(event):
    ...
    try:
        ...
        await event.respond(f"🔊 **USER UNMUTED!**\n\n👤 {target_name} ({target_username})")
    except Exception as e:
...
async def hello_handler(event):
    ...
    await event.respond(f'👋 Namaste {sender.first_name}! Kaise ho bhai? 😊')
    raise events.StopPropagation
